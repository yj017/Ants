...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if self.upper_bound==5 or self.upper_bound==6:
        ...
        if temp!=None:
            while not temp.is_hive and count<=self.lower_bound:
                if temp.bees!=[]:
                    return random_bee(temp.bees) # REPLACE THIS LINE
                temp=temp.entrance
                count+=1
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        while not temp.isAntHomeBase:
            if temp.ant!=None:
                if temp.ant.is_container and not temp.ant.blessing_from_queen:
                    temp.ant.damage*=2
                    temp.ant.blessing_from_queen=True
                    if temp.ant.ant_contained!=None and not temp.ant.ant_contained.blessing_from_queen:
                        temp.ant.ant_contained.damage*=2
                        temp.ant.ant_contained.blessing_from_queen=True
                elif temp.ant.is_container:
                    if not temp.ant.ant_contained.blessing_from_queen:
                        temp.ant.ant_contained.damage*=2
                        temp.ant.ant_contained.blessing_from_queen=True
                else:
                    if not temp.ant.blessing_from_queen:
                        temp.ant.damage*=2
                        temp.ant.blessing_from_queen=True
